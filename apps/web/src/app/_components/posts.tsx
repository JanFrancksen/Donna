	...
	return (
		<div className='flex flex-row rounded-lg bg-white/10 p-4 transition-all hover:scale-[101%]'>
			...
			<div className='flex-grow'>
				<h2 className='font-bold text-2xl text-emerald-400'>{post.title}</h2>
				<p className='mt-2 text-sm'>{post.content}</p>
			</div>
			<div>
				<button
					className='cursor-pointer font-bold text-emerald-400 text-sm uppercase'
					onClick={async () => {
...
export function PostCardSkeleton(props: { pulse?: boolean }) {
	...
	return (
		<div className='flex flex-row rounded-lg bg-white/10 p-4 transition-all hover:scale-[101%]'>
			...
			<div className='flex-grow'>
				<h2
					className={`w-1/4 rounded bg-emerald-400 font-bold text-2xl ${
						pulse && 'animate-pulse'
					}`}
				>
					&nbsp;
